...
void RA_SunLocation::CheckAndUpdate()
{
  ...
  if (m_DayOfMonth != day())
  {
    ...
    if (UseMemory)
    {
      // Write sunrise/sunset to memory
      int oldRiseTime = NumMins(InternalMemory.SunriseHour_read(), InternalMemory.SunriseMinute_read());
      int oldSetTime = NumMins(InternalMemory.SunsetHour_read(), InternalMemory.SunsetMinute_read());

      int riseTime = NumMins(m_SunriseHour, m_SunriseMin);
      int setTime = NumMins(m_SunsetHour, m_SunsetMin);

      if (riseTime / 60 != oldRiseTime / 60) InternalMemory.SunriseHour_write(riseTime / 60);
      if (riseTime % 60 != oldRiseTime % 60) InternalMemory.SunriseMinute_write(riseTime % 60);
