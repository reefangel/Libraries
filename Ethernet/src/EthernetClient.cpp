...
int EthernetClient::noblockconnect(const char* host, uint16_t port) {
  // Look up the host first
  int ret = 0;
  DNSClient dns;
  IPAddress remote_addr;
  ...
  ret = dns.getHostByName(host, remote_addr);
  if (ret == 1) {
    return noblockconnect(remote_addr, port);
  } else {
    return ret;
  }
}

int EthernetClient::noblockconnect(IPAddress ip, uint16_t port) {
  if (_sock != MAX_SOCK_NUM)
    return 0;
...
int EthernetClient::checkconnect() {
  int s=status();
