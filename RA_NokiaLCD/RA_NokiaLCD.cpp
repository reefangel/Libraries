...
void RA_NokiaLCD::DrawSingleMonitorAlarm(int Temp, byte fcolor, byte x, byte y, byte decimal, byte high, byte low, byte wcolor);
{
	int mod=second()%2;
	if (Temp > high || Temp < low) 
	{
		...
		{
			DrawSingleMonitor(Temp, wcolor, x, y, decimal);
		}
	}
	else 
	{
